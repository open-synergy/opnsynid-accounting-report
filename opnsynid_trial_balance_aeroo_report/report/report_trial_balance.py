    ...
    def get_balance(self, type_balance, account_id):
        ...
        if period_id:
            ...
            account_beginning = obj_account.browse(
                self.cr, self.uid, [account_id], context=context_beginning
            )[0]
            account_ending = obj_account.browse(
                self.cr, self.uid, [account_id], context=context_ending
            )[0]
            account_now = obj_account.browse(
                self.cr, self.uid, [account_id], context=context_now
            )[0]
            ...
            if type_balance == "now":
                res = {"debit": account_now.debit, "credit": account_now.credit}
            if type_balance == "ending":
                report_type = account_ending.user_type.report_type
                if report_type in ["income", "liability"]:
                    ending_balance = account_ending.credit - account_ending.debit
                else:
                    ending_balance = account_ending.debit - account_ending.credit
                res = {
    ...
    def get_trial_balance_line(self):
        def _process_child(accounts, parent, level):
            ...
            if account_rec["type"] not in ["view"]:
                ...
                self.total_ending_balance += (
                    self.total_ending_debit - self.total_ending_credit
                )
                ...
                res = {
                    ...
                    "beginning_balance": Decimal(beginning["beginning_balance"]),
                    ...
                    "ending_balance": Decimal(ending_debit["ending_balance"]),
